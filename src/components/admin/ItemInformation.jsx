...
const ItemInformation = ({ isOpen, onClose, item, activeTab }) => {
  ...
  const renderMatchContent = () => {
    const { foundItem = {}, lostItem = {} } = item; // Extract with fallback to empty objects
  ...
  const moveItem = async () => {
    try {
      // Make sure item.id exists (this is the Firestore document ID)
      const docId = item.id;
  ...
  const cancelMatch = async () => {
    try {
      const docId = item.id;
      console.log("Match ID:", docId);  // Check the value of docId before making the request
